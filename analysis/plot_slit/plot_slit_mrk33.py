...
file_name_dict = {
    'mrk33': {
        'file_name_img': '/home/benutzer/data/observation/heII_paper/mrk33/hst_11987_04_wfpc2_f336w_wf_drz.fits',
        ...
        'coords_img': '10h32m31.88s +54d24m02.2s',
...
print('offset_g140l ', ra_offset_g140l, dec_offset_g140l)
...
print('offset_g430m ', ra_offset_g430m, dec_offset_g430m)
...
cutout_img_zoom = helper_func.get_img_cutout(img=img_data, wcs=wcs_img, coord=coords_slit_center_world, cutout_size=(7, 7))
...
plotting_tools.plot_coord_circle(ax=ax_hst_zoom, pos=coords_source_b_world, rad=0.8, color='white',
                                 linestyle='-', linewidth=2, alpha=1., fill=False)
...
ax_hst_zoom.text(coords_source_b_pix[0] - 4, coords_source_b_pix[1] + 7, 'B',
                horizontalalignment='right', verticalalignment='bottom', color='white', fontsize=fontsize)


mean, median, std = sigma_clipped_stats(cutout_img_large.data, sigma=3.0)
# ax_hst_img.imshow(cutout_img_large.data, vmin=mean-std, vmax=mean+200*std, cmap='inferno')
# ax_hst_zoom.imshow(cutout_img_zoom.data, vmin=mean-std, vmax=mean+1000*std, cmap='inferno')
...
ax_hst_img.imshow(np.log10(display_data_img), vmin=-3, vmax=2, cmap='inferno')
