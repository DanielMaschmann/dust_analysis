...
file_name_dict = {
    'ngc_4670': {
        'file_name_img': '/home/benutzer/data/observation/heII_paper/ngc4670/img/hst_06639_03_wfpc2_f439w_pc_drz.fits',
        'filter_name_img': 'F336W',
        ...
        'coords_img': '12h45m17.0s +27d07m31.0s',
...
coords_slit_center_world_b = SkyCoord(file_name_dict[target]['coords_slit_center_b'], unit=(u.hourangle, u.deg))
...
ax_hst_zoom_b = figure.add_axes([0.02, 0.10, 0.4, 0.4], projection=cutout_img_zoom_b.wcs)
...
plotting_tools.plot_coord_circle(ax=ax_hst_zoom_b, pos=coords_source_b_world, rad=0.2, color='white',
                                 linestyle='-', linewidth=2, alpha=1., fill=False)
ax_hst_zoom_b.text(coords_source_b_pix[0] - 2, coords_source_b_pix[1] + 5, 'B',
                horizontalalignment='right', verticalalignment='bottom', color='white', fontsize=fontsize)

coords_source_c_world = SkyCoord(file_name_dict[target]['coords_source_c'], unit=(u.hourangle, u.deg))
coords_source_c_pix = cutout_img_zoom_b.wcs.world_to_pixel(coords_source_c_world)
plotting_tools.plot_coord_circle(ax=ax_hst_zoom_b, pos=coords_source_c_world, rad=0.2, color='white',
                                 linestyle='-', linewidth=2, alpha=1., fill=False)
ax_hst_zoom_b.text(coords_source_c_pix[0] + 2, coords_source_c_pix[1] + 0, 'C',
                horizontalalignment='right', verticalalignment='bottom', color='white', fontsize=fontsize)
...
display_data_zoom_b[((display_data_zoom_b <= 0) | np.isnan(display_data_zoom_b))] = mean
ax_hst_img.imshow(np.log10(display_data_img), vmin=-3, vmax=1.5, cmap='inferno')
ax_hst_zoom.imshow(np.log10(display_data_zoom), vmin=-3, vmax=1.5, cmap='inferno')
ax_hst_zoom_b.imshow(np.log10(display_data_zoom_b), vmin=-3, vmax=1.5, cmap='inferno')
...
ax_hst_img.text(cutout_img_large.data.shape[1] * 0.95, cutout_img_large.data.shape[1] * 0.05,
