...
def plot_slit(ax, coords_slit_pix, slit_length, slit_width, slit_pa, plot_scatter=True, color='red', lw=2, linestyle='-'):
...
def get_single_rgb_img(target, cutout_size=(12, 12), coord_str='coords_img', file_name_str='file_name_r'):

    hdu_hst = fits.open(file_name_dict[target][file_name_str])
    data = hdu_hst[1].data
    wcs = WCS(hdu_hst[1].header)

    coords_cent = SkyCoord(file_name_dict[target][coord_str], unit=(u.hourangle, u.deg))
    cutout = helper_func.get_img_cutout(img=data, wcs=wcs, coord=coords_cent, cutout_size=cutout_size)

    grey_hst = mcf.greyRGBize_image(cutout.data, rescalefn='asinh', scaletype='perc', min_max=[1.0, 99.9],
                                      gamma=file_name_dict[target]['gamma_%s' % file_name_str[-1]], checkscale=False)
    hst_purple = mcf.colorize_image(grey_hst, file_name_dict[target]['color_%s' % file_name_str[-1]], colorintype='hex', gammacorr_color=2.2)
    rgb_hst_image = mcf.combine_multicolor([hst_purple],
                                           gamma=file_name_dict[target]['gamma_rgb'], inverse=False)

    return rgb_hst_image, cutout.wcs


def get_double_rgb_img(target, cutout_size=(12, 12), coord_str='coords_img'):
    hdu_hst_b = fits.open(file_name_dict[target]['file_name_b'])
    hdu_hst_r = fits.open(file_name_dict[target]['file_name_r'])
    hdu_hst_sub = fits.open('/home/benutzer/data/observation/heII_paper/ngc3049/img/HST_11080_a1_NIC_NIC3_F190N_drz.fits')
    data_b = hdu_hst_b[1].data
    data_r = hdu_hst_r[1].data
    data_sub = hdu_hst_sub[1].data
    wcs_b = WCS(hdu_hst_b[1].header)
    wcs_r = WCS(hdu_hst_r[1].header)
    wcs_sub = WCS(hdu_hst_sub[1].header)
    ...
    # wcs_r = fit_wcs_from_points(xy=stars, world_coords=known_coords, projection=wcs_r)
    ...
    data_b = plotting_tools.reproject_image(data=data_b,
                                               wcs=wcs_b,
                                               new_wcs=wcs_sub,
                                               new_shape=data_sub.shape)
    data_r = plotting_tools.reproject_image(data=data_r,
                                               wcs=wcs_r,
                                               new_wcs=wcs_sub,
                                               new_shape=data_sub.shape)
    ...
    cutout_r = helper_func.get_img_cutout(img=data_r, wcs=wcs_r, coord=coords_cent, cutout_size=cutout_size)
    cutout_sub = helper_func.get_img_cutout(img=data_sub, wcs=wcs_sub, coord=coords_cent, cutout_size=cutout_size)


    # cutout_b.data = plotting_tools.reproject_image(data=cutout_b.data,
    #                                            wcs=cutout_b.wcs,
    #                                            new_wcs=cutout_sub.wcs,
    #                                            new_shape=cutout_sub.data.shape)
    # cutout_r.data = plotting_tools.reproject_image(data=cutout_r.data,
    #                                            wcs=cutout_r.wcs,
    #                                            new_wcs=cutout_sub.wcs,
    #                                            new_shape=cutout_sub.data.shape)
    #
    # cutout_b.wcs = cutout_sub.wcs
    # cutout_r.wcs = cutout_sub.wcs
    ...
    # cutout_b.data = np.fliplr(cutout_b.data)
    # hst_img_r = np.fliplr(hst_img_r)
    # cutout_b.data = np.flipud(cutout_b.data)
    # hst_img_r = np.flipud(hst_img_r)




    grey_hst_r = mcf.greyRGBize_image(cutout_r.data, rescalefn='asinh', scaletype='perc', min_max=[1.0, 99.9],
    ...
    rgb_hst_image = mcf.combine_multicolor([hst_r_purple, hst_b_blue],
                                           gamma=file_name_dict[target]['gamma_rgb'], inverse=False)

    return rgb_hst_image, cutout_sub.wcs


def get_rgb_img(target, cutout_size=(12, 12), coord_str='coords_img'):
    hdu_hst_b = fits.open(file_name_dict[target]['file_name_b'])
    hdu_hst_g = fits.open(file_name_dict[target]['file_name_g'])
    hdu_hst_r = fits.open(file_name_dict[target]['file_name_r'])
    data_b = hdu_hst_b[1].data
...
def display_slit(ax, target, wcs, data, slit_dict):
    ...
    if target == 'ngc_4214':
        print(slit_pos_g140l_pix)
        print(slit_pos_g140l_pix[0])
        slit_pos_g140l_pix = np.array(slit_pos_g140l_pix)
        slit_pos_g140l_pix[0] = slit_pos_g140l_pix[0] -2
        print(slit_pos_g140l_pix[0])
...
def display_circ(ax, wcs, coords, text='A', rad=0.2, x_offset=-5, y_offset=7, fontsize=25):
    coords_source_a_world = SkyCoord(coords, unit=(u.hourangle, u.deg))
    coords_source_a_pix = wcs.world_to_pixel(coords_source_a_world)
    plotting_tools.plot_coord_circle(ax=ax, pos=coords_source_a_world, rad=rad, color='white',
...
file_name_dict = {
    ...
    # },
        ...
        'file_name_b': '/home/benutzer/data/observation/heII_paper/ngc3049/img/o54a01010_flt.fits',
        ...
        'coords_img': '9h54m49.6s 9d16m17.0s',
        # 'coords_slit_center': '9h54m49.56s 9d16m17.4s',
        'coords_slit_center': '9h54m49.57s 9d16m16.9s',
        # 'coords_source_a': '9h54m49.50s 9d16m16.5s',
        'coords_source_a': '9h54m49.51s 9d16m16.1s',
        # 'coords_source_b': '9h54m49.613s 9d16m18.07s',
        'coords_source_b': '9h54m49.63s 9d16m17.7s',
    },

    'ngc_3049_band_display': {
        'file_name_r': '/home/benutzer/data/observation/heII_paper/ngc3049/img/o54a01s6q_flt.fits',
...
rgb_image_ngc_3049, wcs_ngc_3049 = get_double_rgb_img(target='ngc_3049', cutout_size=(20, 20*1.5))
rgb_image_ngc_3049_zoom, wcs_ngc_3049_zoom = get_double_rgb_img(target='ngc_3049', cutout_size=(6, 6), coord_str='coords_img')
...
rgb_image_ngc_4670_zoom_2, wcs_ngc_4670_zoom_2 = get_rgb_img(target='ngc_4670', cutout_size=(3, 3), coord_str='coords_source_b')
...
# fontsize = 35
# ax_img_r = figure.add_axes([0.05, 0.05, 0.52, 0.52], projection=wcs_ngc_3049)
# ax_img_g = figure.add_axes([0.53, 0.05, 0.52, 0.52], projection=wcs_ngc_3049_zoom)
#
# ax_img_r.imshow(rgb_image_ngc_3049)
# ax_img_g.imshow(rgb_image_ngc_3049_zoom)
#
# display_slit(ax=ax_img_r, target='ngc_3049', wcs=wcs_ngc_3049, data=rgb_image_ngc_3049[:,:,0],
#              slit_dict=slit_dict_ngc_3049)
# display_slit(ax=ax_img_g, target='ngc_3049', wcs=wcs_ngc_3049_zoom, data=rgb_image_ngc_3049_zoom[:,:,0],
#              slit_dict=slit_dict_ngc_3049)
# display_circ(ax=ax_img_g, wcs=wcs_ngc_3049_zoom, coords=file_name_dict['ngc_3049']['coords_source_a'],
#              text='A', x_offset=-5, y_offset=7, fontsize=25)
# display_circ(ax=ax_img_g, wcs=wcs_ngc_3049_zoom, coords=file_name_dict['ngc_3049']['coords_source_b'],
#              text='B', x_offset=-5, y_offset=7, fontsize=25)
# display_bands(ax=ax_img_r, target='ngc_3049', target_name='NGC 3049', data=rgb_image_ngc_3049[:,:,0], fontsize=fontsize)
#
# plt.show()
#
# exit()
...
ax_img_ngc_3049 = figure.add_axes([0.50, 0.76, 0.52, 0.23], projection=wcs_ngc_3049)
ax_img_ngc_3049_zoom = figure.add_axes([0.60, 0.77, 0.1, 0.1], projection=wcs_ngc_3049_zoom)
ax_img_ngc_3125 = figure.add_axes([0.01, 0.515, 0.52, 0.23], projection=wcs_ngc_3125)
ax_img_ngc_3125_zoom = figure.add_axes([0.31, 0.52, 0.1, 0.1], projection=wcs_ngc_3125_zoom)
ax_img_mrk33 = figure.add_axes([0.50, 0.515, 0.52, 0.23], projection=wcs_mrk33)
ax_img_mrk33_zoom = figure.add_axes([0.80, 0.54, 0.1, 0.1], projection=wcs_mrk33_zoom)
...
ax_img_tol1924_416 = figure.add_axes([0.01, 0.025, 0.52, 0.23], projection=wcs_tol1924_416)
ax_img_tol1924_416_zoom = figure.add_axes([0.25, 0.03, 0.1, 0.1], projection=wcs_tol1924_416_zoom)
ax_img_tol89 = figure.add_axes([0.50, 0.025, 0.52, 0.23], projection=wcs_tol89)
ax_img_tol89_zoom = figure.add_axes([0.85, 0.04, 0.1, 0.1], projection=wcs_tol89_zoom)
...
display_circ(ax=ax_img_mrk33_zoom, wcs=wcs_mrk33_zoom, coords=file_name_dict['mrk33']['coords_source_a'],
             text='A', rad=0.4, x_offset=-2, y_offset=4, fontsize=25)
display_circ(ax=ax_img_mrk33_zoom, wcs=wcs_mrk33_zoom, coords=file_name_dict['mrk33']['coords_source_b'],
             text='B', rad=0.4, x_offset=-2, y_offset=4, fontsize=25)
...
display_circ(ax=ax_img_ngc_3049_zoom, wcs=wcs_ngc_3049_zoom, coords=file_name_dict['ngc_3049']['coords_source_b'],
             text='B', rad=0.4, x_offset=-2, y_offset=4, fontsize=25)
display_bands(ax=ax_img_ngc_3049, target='ngc_3049_band_display', target_name='NGC 3049', data=rgb_image_ngc_3049[:,:,0], fontsize=fontsize)
...
display_circ(ax=ax_img_ngc_3125_zoom, wcs=wcs_ngc_3125_zoom, coords=file_name_dict['ngc_3125']['coords_source_a'],
             text='A', rad=0.1, x_offset=-5, y_offset=7, fontsize=25)
...
display_circ(ax=ax_img_ngc_4214_zoom, wcs=wcs_ngc_4214_zoom, coords=file_name_dict['ngc_4214']['coords_source_a'],
             text='A', rad=0.3, x_offset=-5, y_offset=7, fontsize=25)
...
display_circ(ax=ax_img_tol89_zoom, wcs=wcs_tol89_zoom, coords=file_name_dict['tol89']['coords_source_a'],
             text='A', rad=0.5, x_offset=-5, y_offset=7, fontsize=25)
